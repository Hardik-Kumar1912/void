...
export default function Home() {
  ...
  // Shared CommandPanel props — identical for both mobile and desktop instances
  const commandPanelProps = {
    ...
    logs: buddy.logs,
    // Multi-turn chat props
    messages: buddy.messages,
    isRevisionMode: buddy.isRevisionMode,
    revisionPrompt: buddy.revisionPrompt,
    onRevisionPromptChange: buddy.setRevisionPrompt,
    onStartRevision: () => void buddy.startRevision(),
    onResetConversation: () => void buddy.resetConversation(),
  };
      ...
      <div className="flex-1 flex flex-col md:flex-row p-4 min-h-0 overflow-hidden relative gap-4 md:gap-0">
        ...
        <div className="w-full flex flex-col md:hidden flex-1 min-h-0">
          <div className="flex-1 min-h-0 flex flex-col overflow-hidden">
            {mobileTab === "prompt" && (
              <CommandPanel {...commandPanelProps} />
            )}
            {mobileTab === "code" && (
              <CodeWorkspace
          ...
          <div className="h-[60px] bg-[#0a0a0a] border-t border-zinc-800 flex items-center justify-around shrink-0 -mx-4 -mb-4 mt-4 px-2">
            ...
            >
              ...
              <span className="text-[10px] font-bold uppercase tracking-wider">
                {buddy.isRevisionMode ? "Chat" : "Prompt"}
              </span>
            </button>
            <button
